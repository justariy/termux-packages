...
TERMUX_PKG_DESCRIPTION="A Perl interface to APT's libapt-pkg which provides modules for configuration file/command line parsing, version comparison, inspection of the binary package cache and source package details"
...
termux_step_pre_configure () {
	# pwd is needed by build, restore in post_make_install
	...
	if [ ! -d "$TERMUX_COMMON_CACHEDIR/perl-$TERMUX_ARCH" ]; then
	    termux_error_exit "$TERMUX_COMMON_CACHEDIR/perl-$TERMUX_ARCH doesn't exist. This probably means you need to (re)build perl for $TERMUX_ARCH"
	fi
	$TERMUX_COMMON_CACHEDIR/perl-$TERMUX_ARCH/miniperl_top Makefile.PL \
							       INSTALLDIRS=perl \
							       INSTALLMAN1DIR=$TERMUX_PREFIX/share/man/man1 \
